...
- **图片生成**: 使用 Kolors 等模型生成图片，支持 URL 或 base64 返回
- **多模态支持**: 复用对话消息格式，便于复杂任务处理

#### 🔊 语音合成（TTS，Edge TTS）
- **高质量语音**: 集成微软 Edge TTS，支持多语言、多风格
- **丰富语音库**: 支持晓晓、云健、Aria、Jenny 等 400+ 种语音模型
- **Prosody 参数**: 支持语速(rate)、音量(volume)、音调(pitch)调节
- **流式输出**: 支持流式 TTS 输出，适合 LLM 对话场景
- **多种格式**: 输出 MP3 格式音频
...
- **获取API Key**:
  1. 完成注册并登录
  2. 进入控制台
  3. 在 [API密钥管理](https://cloud.siliconflow.cn/account/ak) 页面创建新的 API Key
  4. 复制生成的 API Key
...
data:
  ...
  pitch: "+0Hz"    # 音调调节，如 "+5Hz" 或 "-5Hz"
  rate: "+0%"      # 语速调节，如 "+10%" 或 "-10%"
  volume: "+0%"    # 音量调节，如 "+10%" 或 "-10%"
...
- **音调(pitch)**: +0Hz（默认，可调节如 +5Hz/-5Hz）
- **语速(rate)**: +0%（默认，可调节如 +10%/-10%）
- **音量(volume)**: +0%（默认，可调节如 +10%/-10%）
- **流式输出**: 支持
...
├── config_flow.py       # 配置流程
├── const.py             # 常量定义
├── conversation.py      # 对话助手
├── ai_task.py           # AI 任务
├── ai_automation.py     # AI 自动化
